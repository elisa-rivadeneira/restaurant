            ...
            <div class="card">
                ...
                <div class="row card-body">
                    ...
                    <div class="form-group col-6">
                        ...
                        <input id="precio_total" type="text" class="form-control" name="costo"  value="{{ $menu->costo }}" readonly>

                    </div>

                   <div class="form-group col-6">
                        <label for="">Porciones</label>
                        <input id="porciones" type="text" class="form-control" name="porciones" value="{{ $menu->porciones }}">
                    </div>
                </div>
            </div>
        ...
        <div class="col-12">
            <div class="card">
                ...
                <table class="table">
                    ...
                    @foreach($menuinsumos as $value)

                    <tr id="tr-{{$value->id}}">
                        <td>
                            <input type="hidden" name="insumo_id[]" value="{{$value->insumo_id}}" />
                            <input type="hidden" name="cantidades[]" value="{{$value->cantidad}}" />
                            {{$value->nombre}}
                        </td>
                        <td>{{$value->cantidad}}</td>
                        <td>{{$value->costo}}</td>
                        <td>{{$value->cantidad * $value->costo}}</td>
                        <td>
                            <button type="button" class="btn btn-danger" onclick="eliminar_insumo({{$value->id}}, {{$value->cantidad}} * {{$value->costo}})">X</button>
                        </td>
                    </tr>
...
//        let costoporcion = costoporciones / porciones;
    ...
    function eliminar_insumo(id, subtotal) {
        ...
        $("#tr-" + id).remove();
        let precio_total = $("#precio_total").val() || 0;
        $("#precio_total").val(parseFloat(precio_total) - subtotal/porciones );
    }
